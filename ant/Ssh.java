...
public class Ssh
    ...
    try {
      ...
      if (ssh == null) {
        ...
        ssh.connect(properties,
                    new AbstractKnownHostsKeyVerification(new File(System.
            getProperty("user.home"), ".ssh" +
            File.separator + "known_hosts").getAbsolutePath()) {
          public void onUnknownHost(String hostname,
                                    SshPublicKey key) throws
              InvalidHostFileException {
            if (Ssh.this.verifyhost) {
              if (key.getFingerprint().equalsIgnoreCase(
                  Ssh.this.fingerprint)) {
          ...
          public void onHostKeyMismatch(String hostname,
                                        SshPublicKey allowed,
                                        SshPublicKey supplied) throws
              InvalidHostFileException {
            if (Ssh.this.verifyhost) {
              if (supplied.getFingerprint().equalsIgnoreCase(
                  Ssh.this.fingerprint)) {
  ...
  public class Shell
    ...
    protected void allocatePseudoTerminal(SessionChannelClient session) throws
        BuildException {
      try {
        if (term != null) {
    ...
    protected void performTasks(SessionChannelClient session) throws
        BuildException {
    ...
    public class Read {
      ...
      public void execute() throws BuildException {
        try {
          ...
          if (output.waitForString(taskString, timeout, new SessionOutputEcho() {
           public void echo(String msg) {
            ...
            throw new BuildException("Timeout waiting for string " + taskString);
          }
        }
        catch (InterruptedException ex) {
          throw new BuildException(ex);
